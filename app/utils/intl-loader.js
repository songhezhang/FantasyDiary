  ...
  sv(callback, force = false) {
    if (!window.Intl || force) {
      require.ensure([
        ...
        const lang = require('data/sv');
        return callback(lang);
      });
    } else {
      require.ensure(
        [ 'data/sv' ],
        (require) => callback(require('data/sv'))
      );
    }
  },

  zh(callback, force = false) {
    if (!window.Intl || force) {
      require.ensure([
        'intl',
        ...
        const lang = require('data/zh');
        return callback(lang);
      });
    } else {
      require.ensure(
        [ 'data/zh' ],
        (require) => callback(require('data/zh'))
      );
    }
  }
